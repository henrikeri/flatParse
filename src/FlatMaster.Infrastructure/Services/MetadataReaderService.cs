...
public sealed partial class MetadataReaderService(
    ...
    IMemoryCache cache,
    IOptions<MetadataReaderOptions> options) : IMetadataReaderService
{
    private readonly ILogger<MetadataReaderService> _logger = logger;
    private readonly IMemoryCache _cache = cache;
    private readonly MetadataReaderOptions _options = options.Value;
    ...
    private async Task<string?> ExtractXisfXmlHeaderAsync(string filePath, CancellationToken cancellationToken)
    {
        ...
        // Keep scan reads small to reduce I/O bursts and antivirus contention during large library scans.
    ...
    private static Dictionary<string, string> ParseXisfXml(string xml)
    {
        var headers = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);

        // Simple XML parsing for FITS keywords and properties
        var fitsKeywordPattern = MyRegex();
        var propertyPattern = MyRegex1();
    ...
    [GeneratedRegex(@"<FITSKeyword\s+(?:name|keyword)=""([^""]+)""\s+value=""([^""]+)""", RegexOptions.IgnoreCase, "nb-NO")]
    private static partial Regex MyRegex();
    [GeneratedRegex(@"<Property\s+id=""([^""]+)""\s+value=""([^""]+)""", RegexOptions.IgnoreCase, "nb-NO")]
